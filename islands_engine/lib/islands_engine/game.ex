defmodule IslandsEngine.Game do
  ...
  def start_link(_args, name), do: start_link(name)
  ...
  def handle_call({:add_player, name}, _from, state_data) do
    ...
    do
      ...
      :error -> reply_error(state_data, :unknown_error)
    end
  end
  ...
  def handle_call({:set_islands, player}, _from, state_data)
  do
    board = player_board(state_data, player)
  ...
  defp update_guesses(state_data, player_key, hit_or_miss, coordinate) do
    update_in(state_data[player_key].guesses,
      fn guesses -> Guesses.add(guesses, hit_or_miss, coordinate) end)
  end
